...
def run_instances(
    ...
    for _ in range(to_start_count):
        ...
        try:
            instance_id = utils.launch(**launch_kwargs)
        except Exception as e:  # pylint: disable=broad-except
            logger.warning(f"run_instances error: {e}")
            logger.warning(f"Launch arguments: {launch_kwargs}")
            raise
    ...
    while True:
        ...
        logger.info(
            f"Waiting for instances to be ready: ({ready_instance_cnt}/{config.count})."
        )
...
def query_ports(
    ...
    while True:
        ...
        if time.time() - start_time > QUERY_PORTS_TIMEOUT_SECONDS:
            logger.warning(
                f"Querying ports {ports} timed out. Ports "
