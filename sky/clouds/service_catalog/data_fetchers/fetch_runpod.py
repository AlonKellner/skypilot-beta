...
def get_partial_runpod_catalog(is_secure: bool) -> pd.DataFrame:
    ...
    runpod = raw_runpod[(raw_runpod["gpu_id"] != "unknown") & (~raw_runpod["gpu_id"].isna())].copy()
    ...
    runpod_ids = set(runpod["gpu_id"].unique())
    ...
    runpod["AcceleratorName"] = runpod["gpu_id"].replace(REVERSE_GPU_MAP)
