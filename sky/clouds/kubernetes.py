...
class Kubernetes(clouds.Cloud):
    ...
    def _unsupported_features_for_resources(
        ...
        is_exec_auth, message = kubernetes_utils.is_kubeconfig_exec_auth(context)
        if is_exec_auth:
            ...
            unsupported_features[clouds.CloudImplementationFeatures.AUTODOWN] = message
    ...
    @classmethod
    def _log_unreachable_context(
        cls, context: str, reason: Optional[str] = None
    ) -> None:
        ...
        if existing_contexts and all(
            ctx in cls.logged_unreachable_contexts for ctx in existing_contexts
        ):
            logger.warning(
    ...
    def regions_with_offering(
        ...
        if (
            autoscaler_type is not None
            and not kubernetes_utils.get_autoscaler(autoscaler_type).can_query_backend
        ):
        ...
        for r in regions:
            ...
            if autoscaler.can_create_new_instance_of_type(context, instance_type):
                logger.debug(
                    f"Kubernetes cluster {context} can be "
    ...
    def get_default_instance_type(
        ...
        instance_cpus = (
            float(cpus.strip("+")) if cpus is not None else cls._DEFAULT_NUM_VCPUS
        )
    ...
    def get_accelerators_from_instance_type(
        ...
        return (
            ...
            if (
                inst.accelerator_count is not None and inst.accelerator_type is not None
            )
            else None
        )
    ...
    def _calculate_provision_timeout(num_nodes: int) -> int:
        ...
        return int(
            min(base_timeout + (per_node_timeout * (num_nodes - 1)), max_timeout)
        )
    ...
    def make_deploy_resources_variables(
        ...
        if acc_count > 0 and acc_type is not None:
            ...
            avoid_label_keys = kubernetes_utils.get_accelerator_label_keys(context)
        ...
        if isinstance(remote_identity, dict):
            ...
            if k8s_service_account_name is None:
                err_msg = (
                    f"Context {context!r} not found in "
        ...
        skypilot_freeze_lines = (
            subprocess.run(
                ["bash", "-c", "pip freeze | grep skypilot"], stdout=subprocess.PIPE
            )
            .stdout.decode("utf-8")
            .splitlines()
        )
        if len(skypilot_freeze_lines) > 1:
            raise exceptions.SkyPilotError(
                "Multiple skypilot packages found in pip freeze output. "
                "Please ensure only one skypilot package is installed."
            )

        nightly_versions = [
            ("skypilot-nightly[kubernetes,remote]" + line.split("skypilot-nightly")[-1].split("]")[-1])
            for line in skypilot_freeze_lines
            if "nightly" in line
        ]
        non_nightly_versions = [
            ("skypilot[kubernetes,remote]" + line.split("skypilot")[-1].split("]")[-1])
            for line in skypilot_freeze_lines
            if "nightly" not in line
        ]

        skypilot_version = [*nightly_versions, *non_nightly_versions][0]
        ...
        deploy_vars = {
            ...
            "k8s_high_availability_storage_class_name": (k8s_ha_storage_class_name),
    ...
    def _get_feasible_launchable_resources(
        ...
        if resources.instance_type is not None:
            ...
            return resources_utils.FeasibleResources(
                [resources], fuzzy_candidate_list, None
            )
        ...
        default_instance_type = Kubernetes.get_default_instance_type(
            cpus=resources.cpus, memory=resources.memory, disk_tier=resources.disk_tier
        )
        ...
        if accelerators is None:
            ...
            k8s_instance_type = (
                kubernetes_utils.KubernetesInstanceType.from_instance_type(
            ...
            chosen_instance_type = (
                kubernetes_utils.KubernetesInstanceType.from_resources(
                    gpu_task_cpus, gpu_task_memory, acc_count, acc_type
                ).name
            )
        ...
        # resources.
        return resources_utils.FeasibleResources(
            _make([chosen_instance_type]), [], None
        )
    ...
    def _check_compute_credentials(cls) -> Tuple[bool, Optional[str]]:
        ...
        if reasons is not None:
            formatted = "\n".join(
                [reasons[0]] + [f"{cls._INDENT_PREFIX}" + r for r in reasons[1:]]
            )
        ...
        if not existing_allowed_contexts:
            ...
            return (
                False,
                "No available context found in kubeconfig. "
                "Check if you have a valid kubeconfig file" + check_skypilot_config_msg,
            )
        reasons = []
        hints = []
        success = False
        for context in existing_allowed_contexts:
            try:
                ...
                return (
                    False,
                    f"Credential check failed for {context}: "
        ...
        return (
            False,
            "Failed to find available context with working "
            "credentials. Details:\n" + "\n".join(reasons),
        )

    @classmethod
    def _format_credential_check_results(
        cls, hints: List[str], reasons: List[str]
    ) -> str:
    ...
    def validate_region_zone(self, region: Optional[str], zone: Optional[str]):
        ...
        if zone is not None:
            raise ValueError(
                "Kubernetes support does not support setting zone."
    ...
    def is_label_valid(
        ...
        if not key_valid:
            error_msg = f"Invalid label key {label_key} for Kubernetes. {condition_msg}"
        if not value_valid:
            error_msg = (
                f"Invalid label value {label_value} for Kubernetes. {condition_msg}"
            )
